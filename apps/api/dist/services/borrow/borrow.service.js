...
class BorrowService {
    ...
    async getBorrowById(id) {
        ...
        if (!record) {
            throw new app_error_1.AppError(error_codes_1.ErrorCode.BORROW_RECORD_NOT_FOUND, "Borrow record not found");
        }
        return record;
    }
    async createBorrow(data) {
        ...
        return await db_1.default.$transaction(async (tx) => {
            ...
            if (user.status === client_1.UserStatus.BLOCKED) {
                throw new app_error_1.AppError(error_codes_1.ErrorCode.USER_BLOCKED, "User is blocked");
            }
            if (user.currentBorrowCount + bookIds.length > user.borrowLimit) {
                throw new app_error_1.AppError(error_codes_1.ErrorCode.BORROW_LIMIT_EXCEEDED, "Borrow limit exceeded");
            }
            // 2. Validate Books
            ...
            if (books.length !== bookIds.length) {
                throw new app_error_1.AppError(error_codes_1.ErrorCode.BOOK_NOT_FOUND, "One or more books not found");
            }
            for (const book of books) {
                if (book.availableQuantity <= 0) {
                    throw new app_error_1.AppError(error_codes_1.ErrorCode.OUT_OF_STOCK, `Book ${book.title} is not available`);
                }
            }
    ...
    async returnBook(data) {
        ...
        return await db_1.default.$transaction(async (tx) => {
            ...
            for (const borrowItemId of borrowItemIds) {
                ...
                if (!item) {
                    throw new app_error_1.AppError(error_codes_1.ErrorCode.BORROW_RECORD_NOT_FOUND, `Borrow item ${borrowItemId} not found`);
                }
