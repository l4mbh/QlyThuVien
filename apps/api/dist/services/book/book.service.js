...
class BookService {
    ...
    async adjustInventory(bookId, userId, data) {
        ...
        if (newAvailable < 0) {
            throw new app_error_1.AppError(error_codes_1.ErrorCode.VALIDATION_ERROR, "Số lượng khả dụng không đủ (không thể giảm xuống dưới 0).");
        }
        ...
        if (newTotal < borrowedCount) {
            throw new app_error_1.AppError(error_codes_1.ErrorCode.VALIDATION_ERROR, "Tổng số sách không thể nhỏ hơn số lượng sách đang cho mượn.");
        }
