    ...
    constructor() {
        this.getAllCategories = async (req, res, next) => {
            try {
                ...
                res.json({ data: categories, code: error_codes_1.ErrorCode.SUCCESS });
            }
            catch (error) {
                next(error);
            }
        ...
        this.createCategory = async (req, res, next) => {
            try {
                ...
                if (!validation.success) {
                    throw new app_error_1.AppError(error_codes_1.ErrorCode.VALIDATION_ERROR, validation.error.issues[0].message);
                }
                const category = await this.categoryService.createCategory(validation.data);
                res.json({ data: category, code: error_codes_1.ErrorCode.SUCCESS });
            }
            catch (error) {
                next(error);
            }
        ...
        this.deleteCategory = async (req, res, next) => {
            try {
                ...
                res.json({ data: { success: true }, code: error_codes_1.ErrorCode.SUCCESS });
            }
            catch (error) {
                next(error);
            }
        ...
        this.bulkDelete = async (req, res, next) => {
            try {
                ...
                if (!ids || !Array.isArray(ids) || ids.length === 0) {
                    throw new app_error_1.AppError(error_codes_1.ErrorCode.VALIDATION_ERROR, "Invalid or empty IDs list");
                }
