...
class IsbnService {
    async fetchFromGoogleBooks(isbn) {
        ...
        if (!response.ok)
            throw new app_error_1.AppError(error_codes_1.ErrorCode.BOOK_NOT_FOUND, "Google Books API request failed");
        const data = (await response.json());
        if (!data.items || data.items.length === 0) {
            throw new app_error_1.AppError(error_codes_1.ErrorCode.BOOK_NOT_FOUND, "No book found with this ISBN on Google Books");
        }
    ...
    async fetchFromOpenLibrary(isbn) {
        ...
        const response = await fetch(url);
        if (!response.ok)
            throw new app_error_1.AppError(error_codes_1.ErrorCode.BOOK_NOT_FOUND, "OpenLibrary API request failed");
        const data = (await response.json());
        const bookKey = `ISBN:${isbn}`;
        const book = data[bookKey];
        if (!book) {
            throw new app_error_1.AppError(error_codes_1.ErrorCode.BOOK_NOT_FOUND, "No book found with this ISBN on OpenLibrary");
        }
        return {
